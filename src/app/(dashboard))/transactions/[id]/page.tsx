            ...
            <div className="rounded-3xl bg-white p-6 shadow-sm sm:p-8">
                <div className="flex flex-col gap-5 border-b border-slate-100 pb-6 sm:flex-row sm:items-center sm:justify-between">
                    <div className="flex items-center gap-4">
                        <div className="grid size-12 place-items-center rounded-2xl bg-emerald-100 text-emerald-700"><CheckCircle2/></div>
                        <div>
                    ...
                    <Link href="/transactions/new"><Button>Transaksi Baru</Button></Link>
                </div>
                        ...
                        {transaction.items.map((item) => (
                            <div key={item.productId} className="flex items-center justify-between gap-4 rounded-xl bg-slate-50 p-4">
                                <div>
                                    ...
                                    <div className="mt-1 text-xs text-slate-500">{item.sku}- {item.quantity} x {formatCurrency(item.price)}</div>
                                </div>
                ...
                <div className="grid gap-3 border-t border-slate-100 pt-6 text-sm">
                    <div className="flex justify-between"><span className="text-slate-500">Metode Pembayaran</span><strong className="uppercase">{transaction.paymentMethod}</strong></div>
                    <div className="flex justify-between"><span className="text-slate-500">Total</span><strong>{formatCurrency(transaction.total)}</strong></div>
                    <div className="flex justify-between"><span className="text-slate-500">Dibayar</span><strong>{formatCurrency(transaction.paymentAmount)}</strong></div>
                    <div className="flex justify-between"><span className="text-slate-500">Kembalian</span><strong>{formatCurrency(transaction.change)}</strong></div>
                </div>
            </div>
        </div>
    )
